...
def process_hybrid(file_bytes, file_ext, fields, output_csv="output.csv"):
    ...
    for item in inferred_fields:
        if isinstance(item, dict):  # Ensure item is a dictionary
            # Merge required_data and item, ensuring no duplicated keys
            merged_row = {**required_data, **item}
            
            # If there are duplicate keys (e.g., 'item'), remove one
            if 'item' in merged_row and len(set(merged_row.keys())) != len(merged_row):
                del merged_row['item']  # Remove the 'item' from one of the sources
